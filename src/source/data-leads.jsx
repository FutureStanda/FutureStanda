...
// reactive lead-stage store
(function () {
  const subs = new Set();
  window.LeadStore = {
    subscribe(fn) { subs.add(fn); return () => subs.delete(fn); },
    setStage(id, stage) { const l = window.BIZBOOST_LEADS.leads.find(x => x.id === id); if (l) { l.stage = stage; subs.forEach(f => f()); } }
  };
  window.useLeads = function () { const [, f] = React.useReducer(x => x + 1, 0); React.useEffect(() => window.LeadStore.subscribe(f), []); return window.LeadStore; };
})();
