...
interface AutomationSeed extends Automation {
...
const SEED_AUTOMATIONS: AutomationSeed[] = [
  ...
  { id: 'a3', name: 'New Lead Telegram Alert', trigger: 'Lead created', triggerSource: 'Meta / web form', steps: 2, client: null, enabled: true, runCount: 312, lastRun: '4m ago', icon: 'inbox', accent: '#5BCEFA' },
...
function AutoCard({
  ...
  return (
    ...
    >
      ...
      <div className="row between" style={{ borderTop: '1px solid var(--border)', paddingTop: 10, alignItems: 'flex-end' }}>
        <span className="row gap-2" style={{ fontSize: 11, color: 'var(--text-3)' }}>
          {a.enabled ? (
            <>
...
export default function AutomationsPage() {
  const [automations, setAutomations] = useState<AutomationSeed[]>(SEED_AUTOMATIONS)
  const [filter, setFilter] = useState<FilterType>('All')

  // Canvas state
  const [canvasOpen, setCanvasOpen] = useState(false)
  const [canvasAutomation, setCanvasAutomation] = useState<Automation | null>(null)
  ...
  function openCanvas(automation: Automation | null) {
    setCanvasAutomation(automation)
    setCanvasOpen(true)
  }

  function closeCanvas() {
    setCanvasOpen(false)
    setCanvasAutomation(null)
  }

  function handleSave(nodes: CanvasNode[], edges: CanvasEdge[]) {
    // If creating new automation, add it to the list
    if (!canvasAutomation) {
      const triggerNode = nodes.find(n => n.id === 'n_trigger')
      const newAuto: AutomationSeed = {
        id: 'a' + Date.now().toString(36),
        name: 'New automation',
        trigger: triggerNode?.name ?? 'Trigger',
        triggerSource: 'Custom',
        steps: nodes.filter(n => n.kind !== 'trigger').length,
        client: null,
        enabled: true,
        runCount: 0,
        lastRun: 'Never',
      ...
      // Update existing automation's step count
      setAutomations(prev => prev.map(a =>
        a.id === canvasAutomation.id
          ? { ...a, steps: nodes.filter(n => n.kind !== 'trigger').length }
          : a
      ))
    }
    closeCanvas()
      ...
      <div className="page-root">
        ...
        <div className="page-inner" style={{ maxWidth: 1080 }}>
          ...
          <div className="row between" style={{ alignItems: 'flex-end' }}>
            ...
            <button onClick={() => openCanvas(null)} className="btn btn-primary" style={{ height: 40 }}>
          ...
          >
            ...
            <div className="col" style={{ gap: 2 }}>
              ...
              <span style={{ fontSize: 12.5, color: 'var(--text-3)' }}>
                Each automation fires independently per client. Click <strong style={{ color: 'var(--text-2)' }}>Open Canvas</strong> to edit the node graph.
              </span>
            </div>
          </div>
